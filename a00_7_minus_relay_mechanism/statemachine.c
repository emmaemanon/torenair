...
void change_state(void) {
	switch (sys_state) {
		...
		case DEFAULT:
			...
			if (keynumber == KEY_A) { sys_state = CHANGEDATE_Y3; }
			if (keynumber == KEY_B) { sys_state = PRE_WATERMAX_W3; }
			if (keynumber == KEY_C) { sys_state = PRE_WATERCAL; }
		break;
		case HIDDENPAR:
			hidden_par();
		...
		case PRE_WATERMAX_W3:
			...
			if (keynumber == KEY_str) { sys_state = DEFAULT; }
			if ((keynumber == KEY_B) && (button_state == MAYBE_PUSH_STATE)) { sys_state = PRE_CHANGERATE; }
		break;
